  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 lg:gap-8 h-full pb-12 max-w-7xl mx-auto">
      <div className="lg:col-span-1 flex flex-col h-full space-y-4">
        ...
        {/* NEW SUBJECT INPUT AT TOP FOR BETTER ACCESSIBILITY */}
        <div className="px-1">
          <form onSubmit={handleAddSubjectSubmit} className="flex gap-2">
            <input 
              type="text" 
              placeholder="Add new course..."
              className={`flex-1 text-sm rounded-2xl px-4 py-3 outline-none border focus:ring-2 focus:ring-indigo-500 transition-all ${darkMode ? 'bg-slate-900 border-slate-800 text-slate-200 shadow-inner' : 'bg-white border-slate-200 shadow-sm'}`}
            ...
            <button 
              ...
              className="bg-indigo-600 text-white rounded-2xl p-3 shadow-lg hover:bg-indigo-700 transition-all active:scale-95 disabled:opacity-50"
            >
        ...
        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-4">
          {subjects.length === 0 ? (
            <div className="py-10 text-center space-y-3 opacity-60">
              <div className="w-10 h-10 rounded-full border border-dashed border-slate-400 flex items-center justify-center mx-auto text-slate-400">
                <BookOpen className="w-5 h-5" />
              </div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">No subjects yet</p>
            </div>
          ) : (
